...
def rotation(phi):
	for i in range(0,s):
		...
		x=pbt[i][0]
		y=pbt[i][1]
		pbt[i][0]=int((x*math.cos(math.radians(phi)))-(y*math.sin(math.radians(phi))))
		pbt[i][1]=int((x*math.sin(math.radians(phi)))+(y*math.cos(math.radians(phi))))
	print(pbt)
...
window=GraphWin("2016UCP1103_ROTATION	",600,600)           #for viewport(device coordinates)
...
#calling rotation
px=int(input("Enter X-coordinate of pivot point:"))
py=int(input("Enter Y-coordinate of pivot point:"))
phi=float(input("Enter angle of rotation(negative for clockwise direction):"))

translation(-1*px,-1*py)
rotation(phi)
translation(px,py)
